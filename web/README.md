...
| `/circuits/:slug` | 电路详情 | 原理图示意 + 3a 网格坐标表 + 7 部分内容（网表/元件表/布局图/整理步骤/连线表/仪器/验证值） |
| `/prompt` | 提示词生成器 | v1 / v2 双模板切换；填 12 个字段，实时拼出完整提示词，一键复制 |
| `/lint` | 在线检查 | 粘贴 `.cir`，浏览器内跑 16 条规则（9 error / 6 warn / 1 info），PASS/FAIL + 逐条定位 |
