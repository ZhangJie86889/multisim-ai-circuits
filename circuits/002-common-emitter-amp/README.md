...
> 主信号链 `VIN → C1 → Q1 → C2 → RL` 全部排在 **y = 4 同一行**；
> 偏置网络竖排在 **x = 6**（上偏置 R1 在上、下偏置 R2 在下）；
> 射极网络竖排在 **x = 9**（RE1 在上、RE2 在下），CE 并在 RE2 两端。
> 这样 BASE 与 COL 两个多脚节点都落在竖线上，不会出现交叉。

| 标号 | 列 x | 行 y | 旋转 | 摆放说明 |
|------|------|------|------|---------|
| VIN | 0 | 4 | 0° | 最左，正弦信号源 |
| C1 | 3 | 4 | 0° | 输入耦合，+ 极朝右（朝 BASE） |
| R1 | 6 | 2 | 90° | 上偏置：VCC → BASE |
| R2 | 6 | 8 | 90° | 下偏置：BASE → GND（与 R1 同列） |
| Q1 | 9 | 4 | 0° | E 下 / B 左 / C 上 |
| RC | 9 | 2 | 90° | 集电极电阻：VCC → COL（Q1 正上方） |
| RE1 | 9 | 7 | 90° | 射极交流负反馈（不可旁路），Q1 正下方 |
| RE2 | 9 | 9 | 90° | 与 RE1 串成同一条竖线 |
| CE | 12 | 9 | 90° | 并在 RE2 两端（+ 极朝上） |
| C2 | 13 | 4 | 0° | 输出耦合，+ 极朝左（朝 COL） |
| RL | 16 | 4 | 90° | 输出负载：OUT → GND |
| VCC | 6 | 0 | 0° | 顶部电源符号，正对 R1 上端 |
| GND | 9 | 11 | 0° | 底部地符号（R2 / RE2 / CE / RL 共用） |
...
| # | 从 | 到 | 网络标签 | 建议走线方向 |
|---|-----|-----|---------|-------------|
| 1 | XFG1 `+`（或 VIN 正端） | C1 负极侧（输入侧） | `IN` | 短直线（同行，(0,4)→(3,4)） |
| 2 | C1 正极侧 | Q1 基极 (B) | `BASE` | 短直线（同行，(4,4)→(8,4)） |
| 3 | VCC (+12 V) | R1 上端 | `VCC` | 短直线（同列，(6,0)→(6,2)） |
| 4 | R1 下端 | Q1 基极 (B) | `BASE` | 先竖后横（(6,3)→(6,4)→(8,4)） |
| 5 | Q1 基极 (B) | R2 上端 | `BASE` | 先横后竖（(8,4)→(6,4)→(6,7)） |
| 6 | R2 下端 | GND | — | 短直线（同列，(6,8)→GND） |
| 7 | VCC (+12 V) | RC 上端 | `VCC` | 网络标签（免走线，与第 3 条同网） |
| 8 | RC 下端 | Q1 集电极 (C) | `COL` | 短直线（同列，(9,3)→(9,4)） |
| 9 | Q1 集电极 (C) | C2 正极侧 | `COL` | 短直线（同行，(9,4)→(12,4)） |
| 10 | C2 负极侧 | RL 上端 | `OUT` | 短直线（同行，(14,4)→(16,4)） |
| 11 | RL 下端 | GND | — | 先竖后横（(16,5)→(16,11)→(9,11)） |
| 12 | Q1 发射极 (E) | RE1 上端 | `EMIT` | 短直线（同列，(9,5)→(9,6)） |
| 13 | RE1 下端 | RE2 上端 | `EMIT2` | 短直线（同列，(9,8)→(9,9)） |
| 14 | CE 正极 | `EMIT2` | `EMIT2` | 先横后竖（(12,9)→(9,9)） |
| 15 | CE 负极 | GND | — | 先竖后横（(12,10)→(12,11)→(9,11)） |
| 16 | RE2 下端 | GND | — | 短直线（同列，(9,10)→(9,11)） |
| 17 | VIN 负端 / XFG1 `-` | GND | — | 网络标签（免走线） |
| 18 | XSC1 CH A `+` | `IN` | `IN` | 网络标签（免走线） |
| 19 | XSC1 CH B `+` | `OUT` | `OUT` | 网络标签（免走线） |
| 20 | XSC1 CH A `-`、CH B `-` | GND | — | 网络标签（免走线） |
